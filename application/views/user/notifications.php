...
// Desktop: tabel standar
// Mobile : card-stack per notifikasi (lebih readable)
// ============================================================
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Notifikasi — SITLAKEB TKA</title>
    ...
    <style>
        ...
        /* ══════════════════════════════════════════════════════
           TABEL vs CARD — dikontrol CSS (container)
           Visibility per-item dikontrol JS via .card-hidden
        ══════════════════════════════════════════════════════ */

        /* card-list: padding saja, display dikontrol media query */
        .card-list { padding: 0; }

        /* Desktop: tabel tampil, card-list disembunyikan */
        @media (min-width: 769px) {
            .table-desktop { display: block; }
            .card-list     { display: none !important; }
        }

        /* Mobile: tabel disembunyikan, card-list tampil */
        @media (max-width: 768px) {
            .table-desktop { display: none !important; }
            .card-list     { display: block; }

            /* Filter bar susun vertikal */
            .filter-bar {
        ...
        /* Baris tabel hidden saat filter/search tidak cocok */
        tr.row-hidden  { display: none !important; }

        /* Card hidden saat filter/search tidak cocok —
           pakai !important karena .notif-card { display:flex } lebih spesifik */
...
<div class="page-wrapper">
    ...
    <header class="topnav">
        ...
        <div class="topnav-actions">
            <?php if($unread_count > 0): ?>
                <a href="<?= base_url('user/mark_all_read') ?>"
                   class="btn-primary"
    ...
    <main class="page-content">
        ...
        <div class="page-header">
            ...
            <div class="page-subtitle">Riwayat seluruh notifikasi pada akun Anda</div>
        </div>
                            ...
                            <tbody>
                                ...
                                <tr data-status="<?= $n->is_read == 0 ? 'unread' : 'read' ?>"
                                    ...
                                    <td class="cell-action">
                                        <?php if($n->is_read == 0): ?>
                                            <a href="<?= base_url('user/mark_notification_read/'.$n->id) ?>"
...
/* ============================================================
   ...
   Strategi: gunakan INDEX sebagai sumber kebenaran tunggal.
   Tabel row dan card selalu di-sync bersamaan — tidak ada
   isMobile() branching yang bisa race condition.
============================================================ */
(function () {
    var perPage      = 10;
    ...
    /* ── Ambil INDEX item yang lolos filter & search ───────── */
    /* Pakai row sebagai referensi data-attribute (selalu ada),
       card sebagai render target mobile.
       Keduanya di-generate dari loop PHP yang sama → index sama. */
    function getPassingIndices() {
        var passing = [];
        /* Gunakan allRows jika ada, fallback ke allCards */
    ...
    /* ── Apply visibility ke row DAN card berdasarkan index ── */
    ...
    function showPage(page) {
        ...
        /* Sembunyikan SEMUA dulu (row + card) */
        for (var i = 0; i < total_items; i++) {
            setVisible(i, false);
        }

        /* Tampilkan index yang lolos dan dalam range halaman ini */
